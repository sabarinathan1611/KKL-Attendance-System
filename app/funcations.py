    ...
    try:
            ...
            for record in mysql_data:
                ...
                if not existing_record:
                    ...
                    week_off = session_sqlite.query(Week_off).filter_by(emp_id=emp_id, date=today_date).order_by(Week_off.date.desc()).first()
                    if week_off:
                        print('\n\n\n\n',week_off)
                    else:
                        print('\n\n\n\nno week_off\n\n\n\n')
                    ...
                    if is_holiday:
                        ...
                        if atten and atten.inTime==None:
                            leave_check = session_sqlite.query(leave).filter_by(emp_id=emp_id,date=today_date, status='Approved').first()
                            late_check = session_sqlite.query(late).filter_by(emp_id=emp_id,date=today_date, status='Approved').first()

                            if leave_check or late_check:
                                attendance_status = 'Leave'
                                if emp.branch=='FT':
                                    status=check_ft(today_date,emp_id)
                                    if status!=None:
                                        attendance_status=status
                            elif week_off:
                                attendance_status='Week Off'
                                ...
                                # if emp.branch=='FT':
                                #     check_ft(today_date,emp_id)
                                c_off=session_sqlite.query(comp_off).filter_by(emp_id=emp_id).first()
                                if c_off:
                                    attendance_status='C Off'
                                    session_sqlite.delete(c_off)
                                    session_sqlite.commit()
                                else:
                                    check_leave(today_date,emp_id)
                                    attendance_status = 'Absent'
                                    if emp.branch=='FT':
                                        status=check_ft(today_date,emp_id)
                                        if status!=None:
                            ...
                            if current_shift != emp.shift:
                                ...
                                attendance_status='Wop'
                                new_req=comp_off(emp_id=emp_id,date=today_date)                        
...
def calculate_Attendance_from_db(id):
    ...
    try:
            ...
            if inTime != None:
                # print('shift.shiftIntime : ',shift.shiftIntime)
                # print('inTime.time() : ',inTime.time())
                lateBy = calculate_time_difference(shift.shiftIntime, inTime.time())
                # print('\n\n\n\n\nlate: ',lateBy)
                
                # if lateBy > datetime(2024,1,1,8,0,0).time():
                #     lateBy = None
            else:
                # print('\n\n\n\n\n else part at 851 late: ',lateBy)
                attendance.lateBy = None
            ...
            if outTime is not None:
                attendance.earlyGoingBy = calculate_time_difference(outTime.time(), shift.shift_Outtime)
...
def check_ft(today_date, emp_id):
    ...
    # attend = Attendance.query.filter_by(emp_id=emp_id).all()1
